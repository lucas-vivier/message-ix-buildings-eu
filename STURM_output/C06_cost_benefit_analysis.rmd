...
```{r}
lifetime_renovation <- 20 #30 # years
...
summary <- wide_data_diff %>%
  ...
  summarize(
    ...
    thermal_comfort_sum = sum(thermal_comfort_EUR * discount_factor) / 1e9,
...
if (file.exists(file)) {
  ...
  summary_2050 - summary_2050[[ref]]
    mutate_at(vars(columns_to_diff), list(Difference = ~ . - .data[[ref]]))

  data$Difference <- data$`Carbon tax` - data[[ref]]
...
rename_list <- c(
  cost_renovation_sum = "Delta cost renovation (Billion EUR)",
  cost_heater_sum = "Delta cost heating system (Billion EUR)",
  cost_heat_sum = "Delta cost fuel (Billion EUR)",
  cost_emission_sum = "Delta cost emission (Billion EUR)",
  thermal_comfort_sum = "Delta thermal comfort (Billion EUR)",
  running_cost = "Delta total cost (Billion EUR)"
)
