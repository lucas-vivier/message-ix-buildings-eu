...
save_path <- "STURM_output/figures/test/Emission_cumulated_GtCO2.png"
plot_histogram(data, column, save_path)

column <- "Delta total cost private (euro/hh/year)"
save_path <- "STURM_output/figures/test/Delta_total_cost_private_euro_hh_year.png"
plot_histogram(data, column, save_path)
```
...
save_path <- "STURM_output/figures/test/Delta_total_cost_private_euro_hh_year_vs_Emission_cumulated_GtCO2.png"
...
temp <- manual_sobol_analysis(scenarios, list_features, y)
``` 
...
p <- df %>%
    mutate(Feature = rename_feature[.data[["Feature"]]]) %>%
