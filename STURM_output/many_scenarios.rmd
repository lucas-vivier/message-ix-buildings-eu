...
```{r}
# Scenario to assess 
scenarios <- c("No additional policy", "All policies" , "Renovation wave quality", "Renovation wave quantity", "No renovation wave")
scenario_names <- data %>%
    filter(descr %in% scenarios) %>%
    select(scenario_name, descr)
scenario_names <- setNames(scenario_names$descr, scenario_names$scenario_name)
...
scenarios <- data %>%
    filter(!grepl(ref, scenario_name))
...
scatter_plots(temp,
            ...
            legend_suffix = "",
            y_label = "Emission saving (%)",
            x_label = "Electrictity variation (%)",
            legend = TRUE,
            presentation = FALSE)
```
...
file <- "2024-06-14_192041_2050_summary_countries.csv"
...
```{r}
data_eu <- data_eu %>%
    mutate(scenario_name = ifelse(scenario_name %in% names(scenario_names), scenario_names[scenario_name], scenario_name))
