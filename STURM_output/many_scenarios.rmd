...
temp <- data %>%
    ...
    mutate(shape = 16) %>%
    mutate(shape = ifelse((sub_heat == "" & learning_rate_heat == "constant"), 17, shape)) %>%
    mutate(shape = ifelse((`_objective_renovation` == "" & `_realization_rate_renovation` == "0.5"), 18, shape))
